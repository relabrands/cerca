...
import { findCredential, doctors } from "@/lib/store"
    ...
    <main className="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm space-y-6">
        ...
        <Card className="border-0 shadow-lg">
          <CardContent className="p-6 space-y-4">
            ...
            <p className="text-center text-xs text-muted-foreground">
              ¿Problemas de acceso?{" "}
              <span className="text-primary underline cursor-pointer">Contactar soporte</span>
            </p>
          </CardContent>
        </Card>
        ...
        <div className="rounded-xl border border-border bg-muted/50 p-4 space-y-1">
          ...
          <button
            type="button"
            onClick={() => { setEmail(demoDoctor.email); setPassword("doctor123") }}
            className="text-sm text-primary underline text-left"
          >
            {demoDoctor.name} — {demoDoctor.email}
          </button>
          <p className="text-xs text-muted-foreground">Contraseña: <span className="font-mono">doctor123</span></p>
        </div>
